    ...
    <div className={cn(
      ...
      {variant !== 'default' && (
        <span className="relative flex h-1.5 w-1.5">
          <span className={cn("animate-ping absolute inline-flex h-full w-full rounded-full opacity-75", variants[variant].split(' ')[0])}></span>
          <span className={cn("relative inline-flex rounded-full h-1.5 w-1.5", variants[variant].split(' ')[2].replace('text-', 'bg-'))}></span>
        </span>
      )}
      {children}
    </div>
